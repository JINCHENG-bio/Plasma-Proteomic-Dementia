...
visual_possion_model2 <- glm_mcog %>%
  ...
  unnest(model_results) %>%
  mutate(model = "visual_quasigamma_model2") %>%
  dplyr::filter(term == "value_scaled") 
...
reaction_possion_model2 <- glm_mcog %>%
  ...
  unnest(model_results) %>%
  mutate(model = "reaction_gamma_model2") %>%
  dplyr::filter(term == "value_scaled")
...
module_reg_cog <- bind_rows(fluid_beta_model2[,c("Network","estimate","std.error","statistic","bonf","cog")],
                           numeric_beta_model2[,c("Network","estimate","std.error","statistic","bonf","cog")],
                           prospective_logistic_model2[,c("Network","estimate","std.error","statistic","bonf","cog")],
                           visual_possion_model2[,c("Network","estimate","std.error","statistic","bonf","cog")],
                           reaction_possion_model2[,c("Network","estimate","std.error","statistic","bonf","cog")])
